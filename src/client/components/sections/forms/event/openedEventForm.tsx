...
import { useUpdateAttendance } from "@/src/lib/hooks/update/events/useUpdateAttendance";
...
export default function UpdateViewerAttendanceForm({
  currentStatus,
  event_id,
}: UpdateViewerAttendanceFormProps): JSX.Element {
  const { control } = useForm<UpdateAttendanceStatusForm>();
  const { newStatus, handleStatusChange, handleSubmit } = useUpdateAttendance(
    currentStatus,
    event_id,
  );
      ...
      >
        ...
        >
          ...
          <Typography component="p" sx={openedEventControlsDescriptionSx}>
            Let the organizer know whether you&apos;re going or still
            interested.
          </Typography>
