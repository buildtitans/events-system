...
import { DB, Notifications } from "@/src/server/src/db/types/db";
...
import { NotificationCreationProcedure } from "@/src/server/src/db/clients/types/types";
...
export class NotificationsClient {
  ...
  private formatNewNotifications(rows: Selectable<Notifications>[]) {
    return rows.map((row) => {
      return {
        user_id: row.user_id,
